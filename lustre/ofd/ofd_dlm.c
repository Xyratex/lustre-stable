...
unsigned int ofd_lockahead_enabled = 0;
CFS_MODULE_PARM(ofd_lockahead_enabled, "i", uint, 0644,
		"enable lockahead");

struct ofd_intent_args_la {
 ...
 */
static enum interval_iter ofd_intent_cb_la(struct interval_node *n, void *args)
{
 ...
 */
static int __ofd_intent_policy_la(struct ldlm_namespace *ns,
				  struct ldlm_lock **lockp,
				  void *req_cookie, ldlm_mode_t mode,
				  __u64 flags, void *data)
{
	...
	*reply_lvb = *res_lvb;
	...
	for (idx = 0; idx < LCK_MODE_NUM; idx++) {
		...
		interval_iterate_reverse(tree->lit_root, ofd_intent_cb_la,
					      &arg);
		if (arg.error) {
			unlock_res(res);
 ...
 * \retval		negative value on error
 */
static int __ofd_intent_policy(struct ldlm_namespace *ns,
...
{
	if (!ofd_lockahead_enabled)
		return __ofd_intent_policy(ns, lockp, req_cookie, mode, flags,
					   data);
	else
		return __ofd_intent_policy_la(ns, lockp, req_cookie, mode,
					      flags, data);
}
