...
[ -z "$SMBCLIENTS" ] &&
	skip_env "need at least two nodes: samba server and samba client" && exit 0
...
cleanup() {
	...
	[[ $SMBSTATUS -eq 0 ]] && do_node $LUSTRE_CLIENT_SMBSRV "service smb start"
	unset CIFSCLIENT
}
...
# setup the cifs
setup_cifs $LUSTRE_CLIENT_SMBSRV $SMBSHARE $SMBCLIMNTPT $SMBUSER $SMBPASSWD $SMBCLIENTS ||
...
test_dbench() {
	...
	do_nodesv $clients "set -x; MISSING_DBENCH_OK=$MISSING_DBENCH_OK \
		...
		DIR=$SMBCLIMNTPT/$tdir/\\\$(hostname) LCTL=$LCTL $cmd 2>&1 | tee $log" &
	pid=$!
