...
recovery_time_min() {
	...
	echo $((2*($CONNECTION_SWITCH_MAX+2*$INITIAL_CONNECT_TIMEOUT)))
}
...
test_64() {
	...
	mount_client $MOUNT3 || error "mount failed"
	...
	#start recovery - wake up target_recovery_oversser
	mount_client $MOUNT2&
	echo "sleep for (recovery_time_soft-1) seconds: $(($time_min-1))"
	sleep $(($time_min-1))
	#extend recovery window:
	#target_handle_connect->check_and_start_recovery_timer->extend_recovery_timer
	mount_client $MOUNT1
	sleep 10
	...
	[ "$completed_clients" = "0/3" ] || error "completed_clients != 0/3: "$completed_clients

	stop_ost
	stop_ost2
	stop_mds
}
run_test 64 "check recovery_hard_time"
