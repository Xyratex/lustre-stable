...
configure_smb() {
	...
	smbconf=$(do_node $smbsrv smbd -b | grep CONFIGFILE | sed -re 's/\s+CONFIGFILE: //g')
...
setup_cifs() {
	...
	local parameter_path=$(do_node $smbsrv "testparm -s --section-name $smbshare --parameter-name path 2> /dev/null")
