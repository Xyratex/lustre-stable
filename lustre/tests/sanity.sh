...
#define OBD_FAIL_TGT_CLIENT_DEL2         0x723
	do_facet ost1 $LCTL set_param fail_loc=0x00000723
...
#define OBD_FAIL_OST_WRITE_DELAY	 0x236
	$LCTL set_param fail_val=10 fail_loc=0x236
	kill -9 $pid
	wait $pid
...
test_280() {
	...
	dd if=/dev/urandom of=$DIR/$tfile bs=1M count=2
	cancel_lru_locks osc
	mount_client $MOUNT2
...
#define OBD_FAIL_LDLM_BL_AST_PAUSE      0x32c
	...
	dd if=/dev/zero of=$DIR2/$tfile bs=1M count=1 &
	pid=$!
	sleep 1
	...
	client_reconnect
	evict=$($LCTL get_param osc.$FSNAME-OST*.state |
